...
public class MessageService
{
    ...
    public String processRequestMessage(String requestXmlData) throws StreamException, IOException, ParseException {
        ...
        else if (baseMessage.getMsgType().equalsIgnoreCase("event"))
        {
            ...
            {
                // 取消关注时修改绑定表中的状态为解除绑定
                this.userBindDao.updateUserBindFlagByOpenId(baseMessage.getFromUserName());
                // 需要修改用户表，修改取消关注时间和状态
                ...
                this.wxUserDao.updateByOpenId(wxUser);
            }
    ...
    private void insertEventMessage2DB(String requestXmlData, BaseEventMessage baseEventMessage)
    {
        ...
        this.eventMessageJournalDao.insertEventMessage2DB(eventMessageJournal);
    ...
    private String getResMessageById(String messageId, BaseMessage baseMessage)
    {
        ...
        ResMessage resMessage = resMessageDao.getApprovedResMessageById(map);
    ...
    private String getResMessageByName(String messageName, BaseMessage baseMessage)
    {
        ...
        ResMessage resMessage = resMessageDao.getResMessageByName(messageName, baseMessage.getToUserName());
        String responseMessageStr = "";
        if (resMessage != null)
        {
            if (resMessage.getMsgType().equalsIgnoreCase("text"))
    ...
    {
        AccessToken accessToken = this.accessTokenDao.getAccessToken(toUser);
        // 调用wxweb工程接口获取用户信息
        ...
        try
        {
            ...
            String userInfoJson = ApiUtil.getUserInfo(accessTokenAndopenId, userInfoUrl);
            if (StringUtils.isNotEmpty(userInfoJson))
            {
                ...
                if (subscribe == 1)
                {
                    ...
                    map.put("accountId", toUser);
                    WxUser tmpWxUser = this.wxUserDao.selectByOpenId(map);
                    if (tmpWxUser == null)
                    {
                        wxUser.setCreateTime(new Date());
                        this.wxUserDao.insertWxUser(wxUser);
                    }
                    else
                    {
                        this.wxUserDao.updateByOpenId(wxUser);
                        this.wxUserDao.deleteUnsubscribeTimeByOpenId(map);
                    }
    ...
    private WxLocation getShortestLocation(LocationMessage locationMessage, String accountId)
    {
        ...
        List<WxLocation> list = this.wxLocationDao.getAllWxLocation(accountId);
